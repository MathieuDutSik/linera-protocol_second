...
use linera_views::sync_views::{SyncRootView as RootView, SyncView as View};
use matching_engine::{OrderId, OrderNature, Parameters, Price, PriceAsk, PriceBid};
use serde::{Deserialize, Serialize};

pub type Context = linera_views::context::ViewSyncContext<Parameters, KeyValueStore>;

pub type CustomCollectionView<K, V> =
    linera_views::sync_views::collection_view::SyncCustomCollectionView<Context, K, V>;

pub type MapView<K, V> = linera_views::sync_views::map_view::SyncMapView<Context, K, V>;

pub type QueueView<T> = linera_views::sync_views::queue_view::SyncQueueView<Context, T>;

pub type RegisterView<T> = linera_views::sync_views::register_view::SyncRegisterView<Context, T>;
