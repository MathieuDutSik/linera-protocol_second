...
fn get_bytecode_path(path: &Path, file_name: &str, contract_name: &str) -> anyhow::Result<Vec<u8>> {
    ...
    let contracts = json_data
        .get("contracts")
        .context(format!("failed to get contracts, json_data={}", json_data))?;
...
contract ExampleCallWasmCounter {
  ...
  function bcs_serialize_CounterRequest(CounterRequest memory input) internal pure returns (bytes memory) {
    bytes memory result = abi.encodePacked(input.choice);
    if (input.choice == 1) {
      return abi.encodePacked(result, bcs_serialize_uint64(input.increment));
    }
  ...
  function bcs_deserialize_offset_CounterRequest(uint256 pos, bytes memory input) internal pure returns (uint256, CounterRequest memory) {
    uint256 new_pos;
    uint8 choice;
    (new_pos, choice) = bcs_deserialize_offset_uint8(pos, input);
    uint64 increment;
    if (choice == 1) {
      (new_pos, increment) = bcs_deserialize_offset_uint64(new_pos, input);
    }
    return (new_pos, CounterRequest(choice, increment));
  }

  function bcs_deserialize_CounterRequest(bytes memory input) public pure returns (CounterRequest memory) {
    uint256 new_pos;
    CounterRequest memory value;
    (new_pos, value) = bcs_deserialize_offset_CounterRequest(0, input);
